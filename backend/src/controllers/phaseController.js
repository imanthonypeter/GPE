...
exports.createPhase = async (req, res) => {
  try {
    ...
    if (project.leader_id !== req.user.id && req.user.role !== 'teacher') {
      return res.status(403).json({ error: 'Only leader or teacher can create phases' });
    }
...
exports.updatePhaseStatus = async (req, res) => {
  try {
    ...
    if (project.leader_id !== req.user.id && req.user.role !== 'teacher') {
      return res.status(403).json({ error: 'Only leader or teacher can update phase status' });
    }
